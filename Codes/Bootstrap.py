...
fraction_of_eligible = np.clip(Pop_of_Eligible / N, 0, NonHesitant / N)
...
"extrapolate")
fraction_of_eligible_interpolated = interpolate.interp1d(time_ref, fraction_of_eligible, kind='zero', axis=0,
                                                         fill_value=
                                                         "extrapolate")



def dot_fraction_of_vaccinated(x, y, alpha_11, cv1, cv_bar0, eta, s_up, s_f, C_i, sigma_11, rate_11,):
    y0, y1 = y
    ...
    if x < time_ref[inArg1]:
        ...
        cv = cv1 * ((x - time_ref[inArg1]) * 0.5 + 1) ** cv_cof
